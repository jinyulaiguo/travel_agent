...
async def get_classic_recommendations(
    city: str, lat: float, lng: float, limit: int = 5, db: AsyncSession = Depends(get_db)
):
    """获取经典餐饮推荐"""
...
async def get_specialties(city: str, db: AsyncSession = Depends(get_db)):
    """获取城市特色菜"""
    return await DiningService.get_city_specialties(db, city)

@router.get("/etiquette", response_model=List[DiningEtiquette])
async def get_etiquette(city: str, db: AsyncSession = Depends(get_db)):
    """获取城市用餐礼仪"""
