...
class CostItem(BaseModel):
    ...
    confidence: ConfidenceLevel = Field(ConfidenceLevel.L4, description="整体置信度级别")
...
class CostSummaryResponse(BaseModel):
    ...
    target_budget_cny: Optional[float] = Field(None, description="用户在意图中设定的目标预算 (CNY)")
    budget_status_message: Optional[str] = Field(None, description="预算对比提示，如 '超出预算约 ¥500'")
