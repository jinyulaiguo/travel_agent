...
const HotelList: React.FC<HotelListProps> = ({ data, onRefresh, onSelectAlternative }) => {
  ...
  // Note: timeleft logic removed as we now use ConfidenceBadge for real-time status
      ...
      {selected_hotel && (
        <div className="selected-hotel-card">
          <div className="card-header">
            ...
            <ConfidenceBadge 
              level={selected_hotel.confidence_level} 
              timestamp={selected_hotel.price_snapshot_time}
            />
          </div>
          ...
          <div className="hotel-info">
            ...
            <div className="price-info">
              <div style={{ display: 'flex', alignItems: 'center', gap: '8px', justifyContent: 'flex-end' }}>
                <p className="price">¥{selected_hotel.price_per_night.value}<span> / 晚</span></p>
                <ConfidenceBadge 
                  level={selected_hotel.price_per_night.confidence_level}
                  timestamp={selected_hotel.price_per_night.snapshot_time}
                  note={selected_hotel.price_per_night.note}
                />
              </div>
              ...
              <div key={hotel.hotel_id} className="alt-card">
                <div className="alt-header">
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: '4px' }}>
                    <span className="alt-price">¥{hotel.price_per_night.value}</span>
                    <ConfidenceBadge level={hotel.price_per_night.confidence_level} />
                  </div>
                </div>
